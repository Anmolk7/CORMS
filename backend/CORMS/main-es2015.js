...
let AuthService = class AuthService {
    ...
    setAuthTime(duration) {
        //console.log("setting duration " + duration);
        this.tokenTime = setTimeout(() => { this.logout(); }, duration * 1000);
    }
    authSave(token, expireDate, userId, username) {
...
let PostCreateComponent = class PostCreateComponent {
    constructor(postService, activeRoute, router, snackBar, authService) {
...
let PostListComponent = class PostListComponent {
    ...
    onJoin(organization, joinButton) {
        console.log(joinButton._elementRef.nativeElement);
        this.postService.getAllMembers().subscribe(roster => {
            this.rosters = roster;
            this.duplicateRoster = this.rosters.find((e) => e.username === this.username && e.organization === organization);
...
let PostService = class PostService {
    ...
    addPosts(name, description, picture) {
        const post = {
            id: null,
            name: name,
    ...
    updatePost(id, name, description, picture) {
        const post = {
            id: id,
            name: name,
            description: description,
...
let UserProfileComponent = class UserProfileComponent {
    constructor(http, authService, postService) {
        this.http = http;
        this.authService = authService;
        this.postService = postService;
    ...
    onSubmit() {
        ...
        this.http.post("http://localhost:3000/api/user/image", formData).subscribe((res) => console.log(res), (err) => console.log(err));
    }
};
